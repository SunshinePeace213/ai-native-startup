...
def test_json_is_formatted_in_place(linter_root, run_hook, edit_payload, project_env):
    ...
    proc = run_hook(
        "auto-format/data.py",
        edit_payload(fixture, tool_name="Write"),
...
def test_yaml_is_formatted_in_place(linter_root, run_hook, edit_payload, project_env):
...
def test_invalid_json_exits_2_with_parse_error(linter_root, run_hook, edit_payload, project_env):
...
def test_broken_prettier_config_is_infrastructure_not_exit_2(
...
def test_malformed_stdin_fails_open(linter_root, run_hook, project_env):
    ...
    proc = run_hook("auto-format/data.py", "not json {", env_overrides=project_env(linter_root))
    assert proc.returncode == 0


def test_missing_prettier_notes_meta_install(tmp_path, run_hook, edit_payload, project_env):
    ...
    proc = run_hook(
        "auto-format/data.py",
        edit_payload(fixture, tool_name="Write"),
        env_overrides=project_env(bare),
    )
    assert proc.returncode == 0
