...
def test_fixable_file_is_formatted_in_place(linter_root, run_hook):
    ...
    proc = run_hook("js_ts.py", edit_payload(fixture), env_for(linter_root))
...
def test_unfixable_error_exits_2_with_rule_and_location(linter_root, run_hook):
...
def test_out_of_repo_file_is_still_formatted(linter_root, run_hook, tmp_path):
...
def test_vendored_path_is_untouched(linter_root, run_hook):
...
def test_malformed_stdin_fails_open(linter_root, run_hook):
    ...
    proc = run_hook("js_ts.py", "not json {", env_for(linter_root))
    assert proc.returncode == 0


def test_missing_binaries_note_meta_install(tmp_path, run_hook):
    ...
    fixture = bare / "x.ts"
    fixture.write_text(FIXABLE_TS)
    proc = run_hook("js_ts.py", edit_payload(fixture), env_for(bare))
    assert proc.returncode == 0
    assert "meta-install" in proc.stderr
    assert fixture.read_text() == FIXABLE_TS


def test_missing_file_fails_open_with_note(linter_root, run_hook):
    """A file deleted between edit and hook is a race, not an error."""
    proc = run_hook("js_ts.py", edit_payload(linter_root / "ghost.ts"), env_for(linter_root))
    assert proc.returncode == 0
