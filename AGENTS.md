...
- **Memory**: `AGENTS.md` is the hub — topic rules live in `.claude/rules/<kebab-case>.md`, path-scoped via `paths:` frontmatter (no `paths:` → loads every session). Never persist memory in `CLAUDE.md` (it only `@`-imports this file) or a new root markdown file. Fetch/record/edit/create contract: [memory-series.md](.claude/rules/memory-series.md).
