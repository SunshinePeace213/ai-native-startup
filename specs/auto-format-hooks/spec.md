...
- `.claude/hooks/auto-format/worktree_create.py` — WorktreeCreate hook: create worktree + install deps; stdout = path only.
- `.claude/hooks/auto-format/worktree_remove.py` — WorktreeRemove hook: remove worktree, delete `worktree-*` branch.
...
- `worktree_create.py`: branch `worktree-<name>` already exists → reuse it (`git worktree add <path> <branch>`); origin fetch fails or no remote → base on local `HEAD`; `bun install`/`uv sync` failure → log, still print the path; stdout must contain the path and nothing else (all git/install output redirected).
- `worktree_remove.py`: worktree path missing → exit 0; branch not matching `worktree-*` → remove worktree but keep the branch; git errors → note + exit 0.
...
- Reference implementation studied via `gh`: [tfriedel/claude-worktree-hooks](https://github.com/tfriedel/claude-worktree-hooks) — stdin `name` field on create, `worktree_path` on remove, stdout purity, `worktree-*` branch cleanup. Its port-hashing and env-copy features are intentionally not adopted.
