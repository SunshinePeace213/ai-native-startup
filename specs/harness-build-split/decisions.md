...
- **Round numbering across runs**: the global report number N (= highest existing `codex-impl-review-round-*.md` + 1) only names reports and picks the review range; an invocation-local attempt counter A (1–2) drives fix-vs-hand-off control flow, so each harness-review invocation performs at most 2 new attempts and a resumed run may start at N=3 with A=1. Invalidated if 2 rounds should be a per-PR lifetime cap.
