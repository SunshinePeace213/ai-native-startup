...
- **Wire three body-sync touchpoints into `/plan-w-team`:** (1) at publish, fill the body's plan links + AC pointer; (2) after each Codex round, update `## Spec-review status` (state) in addition to the relay comment (history); (3) at loop settle, set the Status line and advance the Lifecycle ▲ marker to `Approved` on approval, otherwise leave it at `Spec-review` — the marker only ever points at a real Lifecycle node, so `Needs Human Review` is recorded as a Status value, not under the marker.
