...
- `templates/status.md` carries, per stage (`intake`, `prd`, `brief`): an exact-format state line (`<stage>: <state>`), a log line for each run (date, commit SHAs, lessons-check result — "none" allowed), plus engagement-level fields for `mode:` (`real` | `fixture`), the issue `#N`, branch name, and (after brief) the PR URL — real mode fills the GitHub trail, fixture mode fills the hosting branch + SHAs — so ledger greps can prove either trail.
