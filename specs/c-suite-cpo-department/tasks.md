...
- **Parallel:** true
- **Satisfies:** AC6
- Create `.claude/rules/c-suite/roster.md`: the org chart table — seat, persona name, human/AI, model, what they own, deployed-as — with exact seat/type/model cells: CPO | Sofia Reyes | AI | `session`; PM | Ethan Park | AI | `opus`; UX Researcher | Priya Nair | AI | `sonnet`; UX Designer | Jonas Weber | AI | `sonnet`; UI Designer | Yuki Tanaka | AI | `sonnet`; Design Lead | Daniel Osei | AI | `opus`; Prototype Owner | Human | human | `n/a` (Workflow 3, Claude Design).
...
- `templates/status.md` carries, per stage (`intake`, `prd`, `brief`): an exact-format state line (`<stage>: <state>`) and run-log lines `<stage>-run: date=<date>; sha=<sha>; lessons=<result>` ("none" allowed), plus exact engagement fields `mode:`, `issue:`, `branch:`, `hosting-branch:`, and `PR:`. Real mode fills issue/engagement branch/PR and writes `hosting-branch: none`; fixture mode writes `issue: none`, `branch: none`, `PR: none`, the current branch under `hosting-branch:`, and each stage SHA in its run log.
...
- **Parallel:** true
- **Satisfies:** AC2
- Create a stub command file `.claude/commands/c-suite/cpo-probe.md` (frontmatter description + one-line body), then read its resolved name from the machine-readable discovery surface — the `slash_commands` field of the headless init event, e.g. `claude -p 'ok' --output-format stream-json --max-turns 1 | jq -r 'select(.type=="system" and .subtype=="init") | .slash_commands[]' | grep "cpo-probe"` — never from a model reply.
- Assert the stub resolves under EXACTLY one form (`c-suite:cpo-probe` or `cpo-probe`, one match total); anything else (zero matches, both forms) is a hard failure to investigate before proceeding.
- Record the winning canonical prefix on a greppable `Canonical command prefix:` line in `.claude/rules/c-suite/cpo-operations.md` (coordinate with builder-rules if it lands first), then delete the stub (`mv` to trash, never `rm -rf`).
- Report the winner in the hand-off — `stage-commands` and all user-facing text use it verbatim, and AC2 compares the live `slash_commands` values against this recorded line.
...
- **Model / Effort:** `opus` / `high`
- **Parallel:** false
- **Satisfies:** AC1, AC2, AC3, AC4, AC12, AC13
- Create the three commands in `.claude/commands/c-suite/`, each following the house five-section format (`command-format.md`) with a frontmatter `description` and `argument-hint`:
  - `cpo-intake.md` (`cpo-intake <client-slug> [request…]`): validate the slug against `^[a-z0-9][a-z0-9-]{0,38}[a-z0-9]$` — fixtures `_example-` + valid slug — BEFORE `gh` auth or any other side effect; STOP on violation and quote every interpolation. Detect existing engagement → resume, else scaffold `products/<client-slug>/` from the question-bank skill's templates. **Real mode:** check `gh` auth, create the engagement issue + linked `feat/<N>-<client-slug>` branch + worktree, fill `mode: real`, issue, branch, and `hosting-branch: none`; on every exit update state/run log, commit with `Refs #N`, push the explicit engagement refspec, and create no comment, label, or PR. **Fixture mode:** create no issue/comment/label/branch/worktree/PR and perform no `gh` action; stay on the current branch, fill `mode: fixture`, `issue: none`, `branch: none`, `PR: none`, and the hosting branch; on every exit update state/run log, commit without `Refs #N` or push, and record that commit's SHA in the intake run log. Then AskUserQuestion chooses LIVE vs ASYNC; LIVE grills every question-bank dimension until locked; ASYNC deploys Priya to generate `discovery/question-list.md` and STOPs with send-to-client instructions; re-entrant runs ingest `discovery/client-answers.md`, deploy Priya for gap analysis, re-lock changed dimensions, and mark `prd` and `brief` `stale` when late answers contradict locked requirements. Every exit runs the lessons check and records its result in that run log.
  - `cpo-prd.md` (`cpo-prd <client-slug>`): preflight (intake `done`, else STOP; stale → confirm); deploy Ethan to draft `prd.md`; CPO (session) consistency pass against `requirements.md`; run TWO automatic `prd-review` rounds, reading the verdict only from the report file (never stdout; silence ≠ approval). If round 2 is still `changes-requested`, AskUserQuestion offers exactly one user-authorized final delta round, accept-with-noted-gaps, or needs-human; round 3 is last and re-presents only accept-with-noted-gaps or needs-human. **Real mode:** upsert each digest as the idempotent engagement-issue comment `<!-- prd-review-round-N -->`, apply the needs-human label only when that exit is selected, update the real trail, then commit with `Refs #N` and push the explicit engagement refspec; create no PR. **Fixture mode:** create no issue comment or label and perform no `gh` action; retain reports locally, record the selected gate outcome in `status.md`, commit on the recorded hosting branch without `Refs #N` or push, and record the commit SHA in the PRD run log. Both modes run the lessons check and record its result in that run log.
  - `cpo-brief.md` (`cpo-brief <client-slug>`): preflight (PRD approved, else STOP); deploy Ethan (project brief, copy deck), Jonas (sitemap & flows, section-brief structure, wireframes), Yuki (brand inputs, visual guidance) — parallel where file-disjoint, Jonas→Yuki sequenced on `section-briefs.md`; Daniel reviews the package against the design-standard checklist (blocking findings routed back once). **Real mode:** on approval set `handed-off`, update the real trail, commit with `Refs #N`, push the explicit engagement refspec, open exactly one engagement PR with `Closes #N`, and record its URL; create no issue comment or label. **Fixture mode:** create no issue/comment/label/PR and perform no `gh` action; set `handed-off`, keep `issue: none`, `branch: none`, and `PR: none`, commit on the recorded hosting branch without `Refs #N` or push, and record the commit SHA in the brief run log. Both modes run the lessons check and record its result in that run log.
- Keep these literal trail-contract lines in the commands so validation can assert every mode/action contract directly:
  - intake — `Real trail: issue=create; comments=none; labels=none; commit=Refs #N; push=engagement branch; PR=none` / `Fixture trail: issue=none; comments=none; labels=none; commit=current branch + recorded SHA; push=none; PR=none`
  - PRD — `Real trail: issue=existing; comments=upsert review digest; labels=needs-human only; commit=Refs #N; push=engagement branch; PR=none` / `Fixture trail: issue=none; comments=none; labels=none; commit=current branch + recorded SHA; push=none; PR=none`
  - brief — `Real trail: issue=existing; comments=none; labels=none; commit=Refs #N; push=engagement branch; PR=open once` / `Fixture trail: issue=none; comments=none; labels=none; commit=current branch + recorded SHA; push=none; PR=none`
...
- Every delegation brief passes the explicit `${CLAUDE_PROJECT_DIR}`-rooted paths of BOTH the owning `SKILL.md`(s) and the templates the task needs, instructing the persona to Read the `SKILL.md` first — this brief channel is the complete fallback when `skills:` preloading is unavailable (including under the legacy `disable-model-invocation: true`, which blocks preloading), so a persona never works from blank forms without its quality bar.
...
- **Parallel:** false
- **Satisfies:** AC2
- Verify the three commands resolve under the probe-confirmed prefix by parsing the `slash_commands` init field (same deterministic surface as the probe) — exactly three matches, one prefix form, equal to the `Canonical command prefix:` line in `cpo-operations.md`.
- Verify the knowledge skills do NOT auto-fire and ARE loadable by name; verify each persona agent's `skills:` preload takes effect (spot-check one persona sees its skill content).
- If `autoInvoke: false` is not honored in this Claude Code version, switch the three skills to the repo's proven `disable-model-invocation: true` fallback AND verify the fallback contract end to end: preloading is blocked under that field, so spot-check that a persona deployed with a stage command's delegation brief Reads the owning `SKILL.md` and templates from the brief-carried paths. Note the deviation either way.
...
- Run the full pipeline for the fictional client `_example-bluebird-bakery` (a bakery wanting a brochure website) **in fixture mode on the implementation branch/worktree** — the `_example-` slug means no engagement issue, branch, or PR is created and every stage commit lands directly on `feat/35-c-suite-cpo-department`, so the fixture ships with the build PR.
- One ASYNC intake round (scripted answers file exercises question-list generation + re-entrant ingestion), then LIVE grilling with the user playing the owner until requirements lock; `cpo-prd` through the Codex gate; `cpo-brief` to the full six-file package with Daniel's approval.
- Keep the finished engagement as the fixture; its wireframes must open offline (no external references); its `status.md` must record `mode: fixture`, the hosting branch, per-stage commit SHAs, and each stage's lessons-check result.
...
- **Model / Effort:** `sonnet` / `medium`
- **Parallel:** false
- Author `specs/c-suite-cpo-department/validate.sh` — one consolidated script implementing every per-AC check from acceptance-criteria.md → `## Validation Commands` as its normative contents, with explicit non-zero exit on any failure and a per-AC pass/fail summary. It must assert all three skill files independently block auto-invocation; reject an empty recorded canonical-prefix extraction before checking probe ↔ recorded ↔ live equality; assert the named resume/stale and PRD gate/mode control-flow markers; assert every roster seat's exact human/AI + model cells; assert each agent's exact skill mapping and each command's required `SKILL.md` + template paths; assert one exact locked marker per named question-bank dimension; require a non-empty SHA for each stage, extract the hosting branch from the ledger, and prove every SHA is its ancestor with `git merge-base --is-ancestor`; and require an intake, PRD, and brief run-log line each with a lessons result. Also run the negative case: invoke `cpo-intake` headlessly with a malformed slug and assert it STOPs with no `products/` side effect.
- Run `bash specs/c-suite-cpo-department/validate.sh` and verify each acceptance criterion is met; commit the script with the validation checkpoint.
