...
| `check_spec_completeness.py` | Stop (command-scoped) | Blocks `/harness-layer:harness-plan` from ending on an incomplete `specs/` folder; resolves the root from stdin `cwd`, gating only that session's `specs/` |
