#!/usr/bin/env python3
"""
Validate the YAML frontmatter of a Claude Code slash command
(.claude/commands/<name>.md) or a skill (.../SKILL.md) against the
Claude Code field surface and char caps.
...
KNOWN_KEYS = {
    ...
    "hooks", "paths", "license", "metadata", "compatibility",
}
...
def validate(path):
    ...
    if "description" not in fm:
        warns.append("WARN: no 'description' field (allowed, but without one the command/skill only fires on manual /name invocation)")
    ...
    if "model" in fm:
        ...
        if model not in MODELS:
            fails.append(f"FAIL: 'model' must be an alias or inherit — one of {sorted(MODELS)}: {model!r}")
    ...
    if non_packageable and p.name == "SKILL.md":
        warns.append(f"WARN: Claude Code-only key(s) {non_packageable} are fine in-repo but fail the packaging validator (quick_validate/package_skill)")
...
def main(argv):
    if len(argv) != 2:
        print("Usage: uv run --with pyyaml python scripts/validate.py <path-to-command-or-SKILL.md>")
        return 1
