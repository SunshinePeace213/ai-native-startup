    ...
    for eval_idx, eval_dir in enumerate(sorted(search_dir.glob("eval-*"))):
        ...
        for config_dir in sorted(eval_dir.iterdir()):
            ...
            for run_dir in sorted(config_dir.glob("run-*")):
                ...
                # timing.json is the only source of token counts, so read it
                # whether or not grading.json already carried a duration.
...
def generate_benchmark(benchmark_dir: Path, skill_name: str = "", skill_path: str = "") -> dict:
    ...
    # Count what was actually run rather than asserting a fixed number — the
    # spread is only interpretable when it comes from repeated runs.
    runs_per_config = max(
        (
            len({(r["eval_id"], r["run_number"]) for r in runs_}) // max(len(eval_ids), 1)
            for runs_ in results.values()
        ),
        default=0,
    )
...
def main():
    ...
    if not benchmark["runs"]:
        print(
            f"No graded runs found under {args.benchmark_dir}. Expected "
            "eval-<id>/<config>/run-<k>/grading.json — generate the layout with "
            "`python -m scripts.run_behavior_eval <skill-dir>`."
        )
        sys.exit(1)
