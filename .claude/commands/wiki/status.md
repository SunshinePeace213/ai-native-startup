...
- absorb is computed on every run — it derives from MANIFEST and page `sources:` alone, never from LOG. Only breakdown and cleanup can be insufficient history.
...
- Seed-only wiki (every domain table empty, no lint entries): report zero page counts and "no lint yet" — not an error. absorb still runs, and a seed wiki cites no mirror at all, so it fires whenever MANIFEST holds more than 10 entries.
