---
description: Implement a saved plan on its worktree via a draft PR, then verify it — internal review (1×) then Codex cross-check (≤2×)
...
Follow the `Workflow` to implement the plan at `PATH_TO_PLAN` on the branch `/plan-w-team` drafted it on, drive it through a draft PR, verify it with an internal review then a Codex cross-check, and finish by marking the PR ready and reporting.
...
CROSS_CHECK: `implementation-review` — the Codex cross-check skill, run via `codex exec` (node 8)
...
6. **Internal tidy** — Deploy the tidy subagent for each layer the build touched, on `opus`: `HARNESS_SIMPLIFIER` for harness/prompt files, `CODE_SIMPLIFIER` for application code (both when a build spans both). Behavior-preserving only. Post their one report as the `<!-- report:tidy -->` comment, commit+push, tick **Tidy**.
...
8. **Cross-check** — Have Codex review the implementation against the plan under a SHA freeze, up to `MAX_CROSS_CHECK_ROUNDS`, posting `<!-- report:codex-round-N -->` and fixing blockers each round. Tick **Codex R1**, **Fixes**, and **Codex R2+ delta** as they occur — the R2+ row always holds the latest delta round, and each round appends its own `report:codex-round-N` entry to `## Review Reports`. Gate the finish on the outcome. See `Verification`.
...
**Internal review — node 7 (`INTERNAL_CHECK`, ×`INTERNAL_CHECK_ROUNDS`).** Deploy one read-only review-lead subagent on `sonnet` that executes the official plugin's workflow — read `commands/code-review.md` from the installed `code-review@claude-plugins-official` plugin and follow its steps (Haiku eligibility/context/summary agents, 5 parallel Sonnet reviewers, Haiku confidence scoring, ≥80 filter) — with exactly two caller adaptations it must be told: (a) the draft state is intentional, so the eligibility step must not decline for draft-ness or absence of prior review; (b) it may use **read-only** `gh` retrieval (`gh pr view/diff/list`, `gh issue view`, `gh search`) for the workflow's PR inputs, but it never posts, comments, or mutates — it returns the consolidated findings to you. The plugin's models are the policy: nothing above `sonnet` is spawned for internal review; the tidy simplifiers (`opus`) are the sole exception above Sonnet. YOU then post its findings verbatim as `<!-- report:code-review -->` (stating the reviewed head SHA), apply every finding, and commit+push.

**Codex cross-check — node 8 (`CROSS_CHECK`, ≤ `MAX_CROSS_CHECK_ROUNDS`).** Codex runs a risk-tiered review of the implementation against the plan — does it follow the plan, are the tasks done, do the Validation Commands pass, are there real bugs — and flags simpler/cleaner ideas as advisory. Each round runs under a **SHA freeze**: implementation writes are frozen while the round runs, and any push invalidates the round (re-run it).
...
1. **Ask Codex.** Pass `BASE_SHA`, `REVIEWED_HEAD_SHA`, the round number `N`, and (for N>1) the prior round's reviewed-head SHA explicitly:
   `codex exec -C "<worktree root>" -s workspace-write "Use the implementation-review skill to review round <N> of the plan at specs/<name>/spec.md. BASE_SHA=<base>, REVIEWED_HEAD_SHA=<head>, PRIOR_REVIEWED_HEAD=<prior-or-none>. Round 1 is a full risk-tiered review; round 2+ reviews only the delta <prior-reviewed-head>..<current-head>. Run its Validation Commands, write your verdict to specs/<name>/reviews/codex-impl-review-round-<N>.md, and return only the terse summary."`
