...
7. Define Team Members - Use `ORCHESTRATION_PROMPT` (if provided) to guide team composition — one agent, one purpose. Identify from `TEAM_MEMBERS`, use `GENERAL_PURPOSE_AGENT`, or route a task to Codex when its shape fits the AGENTS.md Codex co-development bullet. Document in plan.
8. Define Step by Step Tasks - Use `ORCHESTRATION_PROMPT` (if provided) to guide task granularity and parallel/sequential structure. Write out tasks with IDs, dependencies, assignments, and each task's implementer (`claude` | `codex`), model, and effort — picked quality-first from the AGENTS.md tables (Quality > time > cost; the full range is live, `fable` and `max` included; never down-tier for cost). A `codex` task also stamps its writable file scope and validation commands so the build can lease it. Document in plan.
