...
Turn a rough problem into options worth reacting to. Before any spec exists, search the codebase and brainstorm ~10 places to intervene — ordered cheapest to most ambitious — so scope is set by picking what resonates, not by committing to the first idea. Discovery only: the page lives in `specs/_explorations/` (gitignored, never committed); the refined prompt built from the picks is the hand-off to `/harness-layer:harness-plan`.

## Variables

DESCRIPTION: $1 — the rough problem ("users churn after onboarding"), or a prior pass's improved prompt
...
- **DISCOVERY ONLY** — no commits, no worktree, no spec files, no issues; write only under `EXPLORATIONS_DIR`. The refined prompt travels by copy-paste.
...
- Do not interview the user or lock scope — that is `/harness-layer:harness-plan`'s grilling. This pass only lays out options.
- End by recommending exactly one next hop, with the refined prompt as its input: resonating picks that are user-facing and need to be seen → `/harness-layer:harness-prototypes`; otherwise → `/harness-layer:harness-plan`.

## Workflow

1. Parse `DESCRIPTION` and derive a kebab-case `<slug>`.
...
3. Author the **Intervention ladder** page per `ARTIFACT_RULES` into `EXPLORATIONS_DIR` — one card per intervention ordered cheapest → most ambitious, a "this resonates" toggle per card with a live pick counter; copy-as-prompt returns only the resonating picks as the refined prompt.
4. Publish the page best-effort via the Artifact tool; on failure note "publish skipped" and continue with the local file.
5. Report per `Report` — the inline ladder, the baseline refined prompt, and the hand-off line.

## Refined Prompt

The deliverable. It must stand alone without the page: the problem restated, the resonating interventions with their code pointers, and what each still needs decided. Only what the search surfaced — no generic advice. When `DESCRIPTION` is a prior pass's improved prompt, carry its constraints forward — enrich them with the picks, never drop them.
...
/harness-layer:<harness-prototypes | harness-plan> "<refined prompt>"
```
