...
- You are the **build lead**: you orchestrate, you do not review your own work, and you never edit files directly. The reviewers (`INTERNAL_CHECK`, `CROSS_CHECK`) are read-only and only report findings — YOU route every fix to a `sonnet` fixer subagent (effort per issue) and own every `git` / `gh` call, including the PR.
...
4. **Implement** — Deploy the team members named in `tasks.md` (or `general-purpose`) to build each task in dependency order, per `.claude/rules/task-tools.md`, each on the model and effort its task stamps in the plan. Commit+push each checkpoint (`Refs #N`) and collect every builder's hand-off.
...
**Otherwise** → merge the two finding sets (dedup by root cause, keep source IDs), make the **report commit** (the round-1 report file), apply ONE combined fix pass over both reports' blockers through `sonnet` fixer subagents (effort per issue), make the **fix commit** (`Refs #N`, never amend), and push both together (one push). Tick **Fixes** with the fix SHA, then run the Codex R2 delta.
